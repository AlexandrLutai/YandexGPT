...
`functions` - это модуль, содержащий вспомогательные функции, используемые в проекте.
...
- `db_ops(db_name: str)`
  ...
  - Возвращает: `cursor` - Курсор для выполнения операций с базой данных.

- `getDateNextWeekday(numberDay: int) -> datetime.date`
  - Получает дату следующего указанного дня недели.
  - Аргументы:
    - `numberDay (int)` - Номер дня недели (0 - Понедельник, 6 - Воскресенье).
  - Возвращает: `datetime.date` - Дата следующего указанного дня недели.

- `getNameDay(day: int) -> str`
  - Получает название дня недели по его номеру.
  - Аргументы:
    - `day (int)` - Номер дня недели (0 - Понедельник, 6 - Воскресенье).
  - Возвращает: `str` - Название дня недели.

- `assignWorkOffsToText(assignWorkOffs: int) -> str`
  - Преобразует значение назначения отработок в текст.
  - Аргументы:
    - `assignWorkOffs (int)` - Значение назначения отработок (0 - Не желательно, 1 - Можно).
  - Возвращает: `str` - Текстовое представление назначения отработок.

- `getDuration(timeFrom: str, timeTo: str) -> int`
  - Получает длительность урока.
  - Аргументы:
    - `timeFrom (str)` - Время начала урока.
    - `timeTo (str)` - Время окончания урока.
  - Возвращает: `int` - Длительность урока в минутах.
